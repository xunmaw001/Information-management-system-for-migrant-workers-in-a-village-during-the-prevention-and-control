<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="预警" prop="yujing">
          <el-select :disabled="ro.yujing" v-model="ruleForm.yujing" placeholder="请选择预警">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="预警" prop="yujing">
	      <el-input v-model="ruleForm.yujing"
                placeholder="预警" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  created() {
	this.ruleForm.pingguriqi = this.getCurDateTime()
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.fengxianpingguOptions = "低风险,中风险,封控人员,隔离人员".split(',')
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zonghepinggu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zonghepingguCrossAddOrUpdateFlag = false;
						 this.parent.search();
