...
export default {
  data() {
	...
	zhaopian : false,
      ...
      rules: {
          fenxiyuanzhanghao: [
                { required: true, message: '分析员账号不能为空', trigger: 'blur' },
          ],
          mima: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `fenxiyuan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.fenxiyuanCrossAddOrUpdateFlag = false;
						 this.parent.search();
