...
export default {
  data() {
	...
	zhaopian : false,
      ...
      rules: {
          caijiyuangonghao: [
                { required: true, message: '采集员工号不能为空', trigger: 'blur' },
          ],
          mima: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `caijiyuan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.caijiyuanCrossAddOrUpdateFlag = false;
						 this.parent.search();
